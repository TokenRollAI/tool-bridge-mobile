...
if (packageJson.version !== '0.21.0') {
  throw new Error(`@tool-bridge/sdk 必须精确锁定 0.21.0，当前为 ${packageJson.version}`)
}
...
if (/interface\s+PreparedDeviceCredential\s*\{[\s\S]*?\burl\?:\s*string;[\s\S]*?\}/.test(deviceTypes)) {
  throw new Error('@tool-bridge/sdk/device 仍允许 credential 覆盖 SDK 生成的 WebSocket URL')
}

console.log('@tool-bridge/sdk/device@0.21.0 入口验证通过：递归 RN import 图仅依赖 partysocket/ws，call-scoped uploadObject/Store URI 与 mailbox processor/journal/delivery 类型存在，credential 不能覆盖 WebSocket URL，且无旧 uploadContextObject 或 Node ws/process.env 泄漏。')
