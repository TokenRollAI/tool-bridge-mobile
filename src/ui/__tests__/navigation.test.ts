...
describe('tab accessibility labels', () => {
  test('六个 tab 使用稳定、唯一且带上下文的可访问名称', () => {
    const tabs = TAB_ORDER.map(name => TAB_OPTIONS[name])
    expect(tabs.map(tab => tab.title)).toEqual(['状态', '信箱', '能力', '媒体', '活动', '设置'])
    expect(new Set(tabs.map(tab => tab.tabBarAccessibilityLabel)).size).toBe(6)
