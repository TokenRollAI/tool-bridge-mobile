---
description: App 内 timer 的输入、SQLite 状态机、确定性调度、启动恢复、撤销竞态与证据上限。
kind: reference
relations:
  requires:
...
code:
  paths:
    - src/capabilities/productivity/timerSchema.ts
    - src/capabilities/productivity/timerCapabilities.ts
    - src/capabilities/productivity/timerController.ts
    - src/capabilities/productivity/notificationAdapter.ts
    - src/storage/migrations/0002_timers.ts
...
- 产品与验收：`llmdoc/product/requirements-and-roadmap.mdx`、
  `llmdoc/delivery/definition-of-done.mdx`、`llmdoc/integration/upstream-and-platform-gaps.mdx`
