---
description: 在线 direct-call 设备本地信箱的 schema、SQLite 内容域、检索、Markdown 图片、提醒和清除语义。
kind: reference
relations:
  requires:
...
2. **真实 Gateway wire**：必须单独验证 `inbox/deliver` 的 expose/call/result、duplicate id 与正文脱敏；
   其他 capability 的 Gateway 或真机结果不能替代。
3. **平台与双端真机**：必须为当前 commit/artifact 验证原生构建、重启持久化、前后台、锁屏、Markdown
...
在没有更高层当前证据时，只能表述为：“设备在线且 direct call 已到达本地 executor 后，本机可安全保存并查询富内容消息；
