...
引用、强调/删除线、行内/块代码、分隔线、链接文字和图片占位到 React Native `Text/View/Image`；
raw HTML 只作为文字，未知 token 不能获得任意组件或属性。

- 每条最多渲染 600 个 block + inline token；超限时降级为可选择的纯文本 Markdown。
- `linkify: false` 保持不变，因此裸 URL 不会自动变成可点击链接。只保留 Markdown 显式链接
  token 的 `href`；原始值和 URL 规范化后都不得超过 2,048 字符，必须是无 userinfo、标准端口且
  非 IP literal 的 HTTPS hostname，可保留 path、query 与 fragment。`http:`、`mailto:`、`tel:`、
  custom scheme 及其他不合规值只显示文字，不获得原生 link 语义。
- 结构合规链接只在用户点按后调用 runtime；runtime 在交给 `Linking.openURL` 前重做同一 URL policy
...
1. **本地内容自动化**：schema、migration、repository、controller、registry、component 与 local runtime
   ...
   单条/全部已读、确定性幂等、commit 后提醒、clear/replay、revision、RN Markdown 白名单、显式 HTTPS 链接的
   点按后系统交接与拒绝边界、图片点击前零网络、
   任意结构合规 HTTPS、安全跨 hostname redirect、inbox hostname 配置链缺失且 media/link 配置未变；注入
   adapter/fake transport 不是系统或 gateway E2E。
2. **SDK 0.21.0 mailbox consumer contract**：真实 SDK processor + fake `expo/fetch` 证明
...
4. **平台与双端真机**：必须为当前 commit/artifact 验证原生构建、重启持久化、前后台、锁屏、Markdown
   布局、长列表/搜索/排序、全部已读、链接真实系统交接/拒绝反馈、图片真实下载/解码/取消/清理、
   跨 hostname redirect、代理/DNS
...
drain 中被 SDK consumer claim 并到达本地 executor 后，本机可安全保存并查询富内容消息；显式 HTTPS 链接
需用户点按后才交给系统，图片需用户逐图点按后按本地 policy 有界加载，并可 best-effort 请求一条固定本地提醒。”
不能表述为“Agent 可随时给离线
...
- `src/inbox/linkOpener.ts`：显式 Markdown 链接的 HTTPS 结构 policy、2,048 字符上限与点按后系统交接。
- `src/runtime/applicationRuntime.ts`、`src/ui/screens/InboxScreen.tsx`、`src/ui/screens/InboxMessageScreen.tsx`、
  `src/ui/components/SafeMarkdown.tsx`：
  revision、查询/未读组合、摘要/展开、RN Markdown 白名单与点按链接的失败反馈。
