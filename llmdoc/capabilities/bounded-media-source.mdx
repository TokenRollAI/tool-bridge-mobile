---
description: 不可信 HTTPS 音频从 URL 校验到私有缓存、播放器输入和资源释放的有界解析契约。
kind: reference
relations:
  requires:
...
code:
  paths:
    - src/capabilities/media/sourcePolicy.ts
    - src/capabilities/media/sourceResolver.ts
    - src/capabilities/media/expoMediaCacheStore.ts
    - src/capabilities/media/controller.ts
    - src/capabilities/media/expoAudioPlaybackPort.ts
---
...
媒体尚未实现的 `objectRef` source 消费，也不证明真机播放、后台、锁屏或音频中断行为。
...
- HTTPS source 仍是本地媒体输入路径，不等于 Tool Bridge 受保护对象。媒体 source 尚未消费 `objectRef`、绑定
