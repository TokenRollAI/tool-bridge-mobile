---
description: Local-only 通知与信箱提醒的固定内容、权限映射、commit 语义和最终原生配置边界。
kind: reference
relations:
  requires:
...
code:
  paths:
    - src/capabilities/productivity/notificationSchema.ts
    - src/capabilities/productivity/notificationCapability.ts
    - src/capabilities/productivity/notificationController.ts
    - src/capabilities/productivity/notificationAdapter.ts
    - src/inbox/controller.ts
    - src/inbox/identifiers.ts
...
`expo-notifications 57.0.15` 创建即时本地通知。`phone/productivity.notify` 在 SDK expose 中保持
realtime-only；只有 `phone/inbox.deliver` 是 `delivery: both`，但 mailbox 投递的仍是信箱写入 operation，
不是把 notify capability 变成异步通知命令。两者都不注册 push token、不接收 remote notification，也不
提供 background reachability；信箱正文存储契约另见
...
只有用户主动点击本地 UI 才调用权限请求；请求前重建 channel，并有独立有界等待。新安装的 Android
可能把“尚未请求”表达为 `denied + canAskAgain=true`，必须映射为 requestable，不能与永久拒绝合并。
...
不证明本地通知真机呈现，也不满足 U-5 真实 Gateway/双端联合验收或 U-6 push。
