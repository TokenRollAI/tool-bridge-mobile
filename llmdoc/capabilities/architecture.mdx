...
- strict 输入只有安全 `purpose`、前/后摄像头和 low/medium/high 质量；probe 同时检查前台、真实相机硬件
  和权限。等待预览、重编码或上传期间离开 `active` 都立即取消，不允许后台或锁屏完成。
...
- 处理后对象固定上传到 `camera/photos/<deviceId>/<sha256(commandId)>.jpg`，禁止覆盖并严格核对 SDK 返回
  ...
  私有文件 URI或虚构的对象过期时间。
- 本地自动化、双端 clean prebuild 与 production Metro 已通过；Android/iOS 原生 build、双端真机相机
  行为、真实 Gateway/storage、系统指示/快门声与 crash 后孤儿文件清理均未验收。
...
`unavailable: probe_failed`。历史 Android clean debug build 只证明对应历史工作树的原生依赖与配置能编译；
本轮相机只有双端 clean prebuild 与 Metro，不可写成 Android/iOS 原生 build。它们都不证明 production
gateway、storage 或真机前台/后台/锁屏行为。信箱还必须把本地自动化、真实
