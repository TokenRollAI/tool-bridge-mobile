...
- strict 输入只有安全 `purpose`、前/后摄像头和 low/medium/high 质量；argument-aware preflight 在本地
  确认前用原生只读枚举拒绝设备不存在的 requested facing。通用 probe 先检查前台，再要求原生枚举至少
  存在一个 `front | back`，随后才读取系统 Camera 权限；native 异常或未知 facing 稳定映射为
  `camera_probe_failed`，不向远端返回原始详情。等待预览、重编码或上传期间离开 `active` 都立即取消，
  不允许后台或锁屏完成。
...
- 处理后对象固定上传到 `camera/photos/<deviceId>/<sha256(commandId)>.jpg`，禁止覆盖并严格核对 SDK 返回
  的稳定 `node://` URI；inline result 只含 URI、MIME、大小、尺寸与 SHA-256，不含照片、signed URL、
  私有文件 URI或虚构的对象过期时间。
- 本地自动化覆盖相机 schema、状态机、处理和 SDK upload consumer；prebuild、Metro、原生 build、双端
  真机、真实 Gateway/storage、系统指示/快门声与 crash 后孤儿清理必须按各自证据层单独判断。
...
- 使用 `expo-notifications 57.0.14` 调度即时 local notification，不注册 push token，也不实现 remote
...
- camera upload context：固定 `camera/photos`，由 Gateway 部署预挂载可写 R2/S3 context 并授予设备凭证
  写 scope，不提供构建时可改的 context path
