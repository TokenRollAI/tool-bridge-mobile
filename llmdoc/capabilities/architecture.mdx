---
description: 当前本地能力切片的共同模式、平台实现、配置来源与证据上限。
kind: architecture
relations:
  requires:
  ...
  related:
    ...
    - capabilities/bounded-linking-handoffs.mdx
    - capabilities/bounded-media-source.mdx
    - capabilities/local-device-inbox.mdx
    - capabilities/local-notifications.mdx
    - capabilities/local-timers.mdx
...
- 仓库自有 Expo native module `modules/tool-bridge-attention` 当前提供 haptic probe/执行、torch 闪光灯
  probe/enable/disable，以及相机前/后镜头的只读枚举。镜头枚举在 Android 走 Camera2
  `CameraManager.cameraIdList` + `CameraCharacteristics.LENS_FACING`，在 iOS 走 AVFoundation
  `AVCaptureDevice.default(... position:)`；只返回 schema 支持的 `front | back`，不创建采集 session、
  不打开采集流、不读取照片，也不请求 Camera 权限。其余已实现能力通过受限 Expo API、App 内状态机或
  系统 handoff 提供。
...
- 当前原生权限面仍按已实现切片最小化：相机切片通过 `expo-camera` 声明 Android CAMERA 与 iOS
  `NSCameraUsageDescription`；未声明麦克风、录音、图库或后台位置权限。Android 更高权限控制路线尚未
...
### `phone/camera.capture_photo`

- descriptor 是 high-risk write、`confirmation: always`、foreground-only、`reject_offline`；Direct call
  由本机全局策略跳过逐次确认，但仍进入全屏可见预览，预览就绪后自动拍摄且没有用户快门/拍后复核。
  Ask every time / Trusted session 则在通用确认后由用户按快门，并允许重拍或确认上传。
- strict 输入只有安全 `purpose`、前/后摄像头和 low/medium/high 质量；argument-aware preflight 先要求
  当前 SDK call 的 Store upload capability，再用原生只读枚举拒绝设备不存在的 requested facing。两项都
  发生在 admission、通用 probe 和本地确认之前，缺上传能力时不会先弹确认或打开相机。通用 probe 先检查
  前台，再要求原生枚举至少存在一个 `front | back`，随后才读取系统 Camera 权限；native 异常或未知
  facing 稳定映射为 `camera_probe_failed`，不向远端返回原始详情。
- 原图最大 30 MiB；本地移除 EXIF 并按质量重编码为最长边 1280/1920/2560 的 JPEG，输出最大 10 MiB，
  复核 JPEG 边界并计算 SHA-256。临时文件在已知成功/失败/取消/重拍路径 best-effort 清理。
- 处理后对象经本次调用的 `call.uploadObject` 上传；consumer 提交 JPEG Blob、真实大小、SHA-256 checksum、
  ...
  或虚构的对象过期时间。
- 离开 `active` 会取消本地相机/处理和结果提交；当前 SDK 0.21.0 的 call-scoped upload 仍不能接收
  controller 的本地 AbortSignal，因此上传已开始后不能保证终止 Store HTTP 副作用。2026-08-26 历史现场
  Gateway 0.17 的约 60 秒权威期限会收紧相机 120 秒本地 ceiling；这不是当前 0.21.0 Gateway 回归证据。
...
完整 schema、UI 状态、处理、上传、部署前置与证据上限见
...
详细解析顺序、MIME 列表和清理所有权见 `llmdoc/capabilities/bounded-media-source.mdx`。
...
完整输入、执行与证据边界见 `llmdoc/capabilities/bounded-linking-handoffs.mdx`。
...
- 使用 `expo-notifications 57.0.17` 调度即时 local notification，不注册 push token，也不实现 remote
...
完整状态转换、恢复条件和证据边界见 `llmdoc/capabilities/local-timers.mdx`。
...
- camera Store upload：没有构建时 context path；只消费 Gateway 为当前 device call 下发的短期 upload
  capability，能力缺失或已过期即 fail closed
...
锁屏行为。信箱还必须把本地内容自动化、SDK 0.21.0 mailbox consumer contract、真实 Gateway operation
...
- P2/P3 候选能力进入 registry 前必须先补用户用例、平台/权限/商店矩阵、abuse case 和独立 DOD；能力
  数量不优先于安全、身份、幂等、审计或端到端可完成性。

产品范围、控制模式与阶段出口见 `product/requirements-and-roadmap.mdx`；所有公开能力的共同完成条件见
`delivery/definition-of-done.mdx`。
