---
description: Command 终态 10,000 总上限、受保护候选、原子写入裁剪与证据边界。
kind: reference
relations:
  requires:
...
code:
  paths:
    ...
    - src/storage/memoryRepositories.ts
    - src/storage/__tests__/commandRepository.test.ts
    - src/storage/__tests__/retention.test.ts
---
...
- 真实 SQLite fixture 对 10,001 个 terminal、1 个 running、1 个 active timer source 执行 production
  等价 DELETE 后必须得到 terminal 10,000，同时保留 running、current 与 protected source，并删除 oldest
  eligible。它验证 SQL 语法/计数，不证明 Expo SQLite 真机锁争用、I/O 或 10,000 条性能。
...
- fixture 与证据记录法：`llmdoc/delivery/verification-evidence.mdx`
- 安全/验收：`llmdoc/runtime/safety-boundaries.mdx`、`llmdoc/delivery/definition-of-done.mdx`
