...
- 大对象必须使用受保护对象流程，不得用公开 URL 或 base64 冒充。相机已通过当前 SDK 0.20.1 调用的窄
  Store capability 接入稳定 `store://default/...` URI；2026-08-26 的 SDK 0.17 Android 真实上传/stat/
  受保护回读是历史现场，不等于 0.20.1 真实 Gateway 回归、通用读取或完整 U-7 服务端强绑定已经验收。
...
- 前台丢失会取消本地相机、处理和结果提交，但当前 SDK 0.20.1 的 call-scoped `uploadObject` 仍不能接收
