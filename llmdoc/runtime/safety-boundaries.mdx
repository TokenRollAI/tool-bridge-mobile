...
- 大对象必须使用受保护对象流程，不得用公开 URL 或 base64 冒充。相机已通过 SDK 0.17 当前调用的窄 Store
  capability 接入稳定 `store://default/...` URI，并完成一次 Android 真实上传/stat/受保护回读；这不等于
  通用读取或完整 U-7 服务端强绑定已经验收。
...
- 前台丢失会取消本地相机、处理和结果提交，但 SDK 0.17 的 call-scoped `uploadObject` 不能接收 controller
  的额外 AbortSignal；Store HTTP 上传已经开始时不能声称其副作用一定被中止。Gateway call signal 仍由 SDK
  自己传播，移动端不得用 `Promise.race` 冒充网络取消。
