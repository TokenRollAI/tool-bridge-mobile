...
- 大对象必须使用受保护对象流程，不得用公开 URL 或 base64 冒充。相机已通过 SDK 0.15 context object
  upload 本地接入稳定 `node://` URI，但真实 Gateway/storage、受保护读取和完整 U-7 服务端强绑定尚未验收。
- 相机照片、私有 `file://` URI、signed upload URL 与 EXIF 不得进入普通日志、audit 或 inline result；结果
  只允许稳定对象 URI及 JPEG 大小/尺寸/SHA-256。移动端必须精确核对 Gateway 返回 URI，不能虚构对象
  `expiresAt`，也不能把本地 path/大小/禁止覆盖当成服务端 grant 强绑定。
...
- 相机仅声明 Camera 权限，不声明麦克风、录音或图库；拍照配置关闭 EXIF，原始/输出分别限制 30/10 MiB，
  ...
  尚无契约。
- 相机上传固定使用 `camera/photos` 和设备凭证 HTTP Authorization；Gateway 必须预挂载可写 context 并配置
  scope。401/403 credential invalidation 只能删除仍与本次完全匹配的旧凭证，不得误删已轮换的新值。
