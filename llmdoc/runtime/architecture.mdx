...
`@tool-bridge/sdk/device@0.17.0` call adapter 驱动；重复投递、用户等待、崩溃和日志处理都不能因
transport 变化产生隐蔽副作用或泄漏敏感参数。

## 核心组件

- `src/runtime/applicationRuntime.ts` (`ApplicationRuntime`)：组装 registry、SQLite、策略、确认、SDK
  transport 与 UI snapshot；AppState 变化时刷新 probe、UI 与策略输入，但 transport 不会因此主动断连。
...
- `src/capabilities/camera/`：`CameraCaptureCoordinator` 只保存一个待处理 UI request，controller 维持
  ...
  SDK 0.17 `call.uploadObject` 并核对 `store://default/...` descriptor。
...
可达证据。持久 command mailbox 与只携带不透明引用的 APNs/FCM wake 仍是上游未实现能力；当前本地服务、
本地信箱和本地通知都不能冒充这条服务端投递链路。
...
- emergency disable 先关闭本地策略闸门、取消进行中 handler、拒绝 pending confirmation，并停止当前
  attention/media session、当前相机流程与活动 timer、suspend SDK realtime；timer revocation epoch 防止已发起但迟到
  的原生调度越过停用边界。
