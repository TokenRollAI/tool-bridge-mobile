...
`@tool-bridge/sdk/device@0.15.0` call adapter 驱动；重复投递、用户等待、崩溃和日志处理都不能因
...
- `src/capabilities/camera/`：`CameraCaptureCoordinator` 只保存一个待处理 UI request，controller 维持
  foreground/cancel/deadline 边界，processor 重编码并清理私有文件，object uploader 通过 SDK 0.15 写入
  固定 `camera/photos`。
...
- 相机照片和私有文件 URI不进入 SQLite、普通 audit、日志、accessibility announcement 或 inline result；
  只有稳定 `node://camera/photos/...`、JPEG MIME、大小、尺寸与 SHA-256 可进入结果。signed upload URL
  只在 SDK helper 内短暂使用，临时照片在已知路径 best-effort 清理，尚无 crash 后孤儿扫描保证。
