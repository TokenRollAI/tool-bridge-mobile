...
capability scope 或 credential-instance binding。0.20.1 SDK call 已提供网关签发的 caller 与权威期限，
...
0.20.1 已交付 durable mailbox 移动 consumer API，本仓库只为 `phone/inbox.deliver` 接入
...
- **未实现**：当前代码和上游 `@tool-bridge/sdk/device@0.20.1` 仍没有 credential-instance binding 与
