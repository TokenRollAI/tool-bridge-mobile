...
code:
  paths:
    ...
    - src/ui/screens/ConnectionSettingsScreen.tsx
    - app/connection.tsx
---
...
正式 pairing/U-2 尚未交付时，设备总览的“连接配置”二级页允许用户手工输入 Gateway HTTPS origin 与 API key，直接驱动
`@tool-bridge/sdk/device@0.21.0` realtime 连接和 active-only mailbox drain。它是内测 fallback，不是
pairing、设备凭证签发、最小 scope、
...
- 同一 envelope 用于认证 device WebSocket session 与显式 mailbox HTTP drain；Store upload 的窄
  capability 由 Gateway 按当前 call 下发，移动 handler 只得到 `call.uploadObject`，不复用长期 API key
  发起旧 context HTTP 上传，也不接触 wire upload token 或 relay URL。
...
validate URL + key + installation identity
  -> realtime/mailbox transports.updateConfiguration(null)
  -> SecureStore.save(envelope)
  -> realtime/mailbox transports.updateConfiguration(new origin)
...
realtime/mailbox transports.updateConfiguration(null)
  -> SecureStore.clear()
  -> realtime/mailbox transports.updateConfiguration(optional build origin)
  -> apply current AppState / control mode
```

SecureStore 写入或删除失败时不得自动重连；错误文案不包含底层 secret。控制器在 Disabled 状态允许本地保存或
清除，但新 transport 保持 suspended，直到用户恢复控制模式；当前连接配置 UI 在停用态隐藏表单并显示恢复提示。
...
- `src/gateway/sdkDeviceMailboxTransport.ts`：动态 gateway origin 与 active-only 显式 drain。
...
unit/component/fake WebSocket/fetch 已覆盖 strict 输入、secret 清空、成功/失败事件顺序、旧 socket 与旧
drain 中止和新 audience header。`pnpm verify` 仍是本地 gate；这些不是实际 Gateway 兼容证据。
