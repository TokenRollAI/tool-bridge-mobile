...
`@tool-bridge/sdk/device@0.15.0` 连接。它是内测 fallback，不是 pairing、设备凭证签发、最小 scope、
...
- 同一 envelope 也供 SDK context object upload 的 HTTP credential provider 使用；每次 create-upload 前
  重新校验 HTTPS audience、deviceId、keyId 与 material，Authorization 只进入 header。create-upload
  401/403 只失效仍与本次完全匹配的 credential，不删除已轮换的新值。
...
相机使用该手工 credential 上传时，Gateway 还必须预挂载可写 `camera/photos` R2/S3 context 并授予写 scope；
这不把长期 API key 变成 pairing、最小 scope 或 production compatibility 证据。
