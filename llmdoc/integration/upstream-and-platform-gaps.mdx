...
证据边界见 `llmdoc/integration/sdk-device-transport.mdx` 和 `docs/SDK.md`。
...
- 本轮 0.15.0 相机工作树的 Android/iOS clean prebuild 与 production Metro/Hermes export 成功；
  prebuild/export 仍不是原生 build 或真机连接证据。
- Android `assembleDebug` 因当前网络访问 Maven Central 返回 HTTP 403 而未通过/未完成；iOS native build
  因本机缺 CocoaPods/完整 Xcode 未运行。两者都不能因 prebuild/autolink/Metro 成功而勾选。
...
- `phone/camera.capture_photo` 目前只有本地自动化、SDK 0.15 consumer contract、双端 prebuild/Metro；
  尚无 Android/iOS 原生 build、双端真机预览/系统指示/快门声/权限/前后台行为或真实
  `camera/photos` Gateway/storage 上传、读取与保留证据。
