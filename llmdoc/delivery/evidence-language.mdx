...
2. **SDK 0.15 upload consumer**：使用发布的 `uploadContextObject` 且真实 helper + mocked fetch 覆盖
   create-upload、HTTP Authorization、Blob PUT 与 credential 精确失效；这不等于真实 Gateway/storage。
...
4. **真实 Gateway/storage**：必须单独验证预挂载可写 `camera/photos`、写 scope、稳定 URI、拒绝覆盖、
   读取与保留；本地确定性 path/10 MiB 不是服务端 command/size/TTL/single-use 强绑定。
5. **双端真机**：需覆盖权限首次请求/拒绝/撤销、前后台/锁屏、真实预览、Direct call 自动物理拍摄、系统
   指示/快门声、上传取消与临时文件；组件测试、prebuild 和 Metro 不能替代。

## 当前可用措辞

- 可写：`@tool-bridge/sdk/device@0.15.0 consumer wiring 已集成`、`新 device call path/context 本地适配已实现`、
  `相机稳定 node URI upload consumer 本地已集成`、
...
- 不可写：`手工 API key 等于已配对`、`已接通 production gateway`、`Agent 可随时给离线手机发信`、
  ...
  `当前 Android/iOS 原生构建已通过`、`已通过真机验收`、`真实 Gateway/storage 已兼容`、
