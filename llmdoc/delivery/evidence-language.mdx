...
- 可写：`@tool-bridge/sdk/device@0.15.0 consumer wiring 已集成`、`新 device call path/context 本地适配已实现`、
  ...
  `本地 executor 已实现持久化去重`、`本轮双端 clean prebuild 与 production Metro 已成功`、
...
- 不可写：`手工 API key 等于已配对`、`已接通 production gateway`、`Agent 可随时给离线手机发信`、
  ...
  `本轮 Android/iOS 原生构建已通过`、`已通过真机验收`、`真实 Gateway/storage 已兼容`、
  `通用 objectRef 读写已完成`、`服务端已强绑定 command/size/TTL/single-use`。

本轮必须明确：Android `assembleDebug` 被 Maven Central HTTP 403 阻塞；iOS native build 因缺 CocoaPods/
完整 Xcode 未运行；双端真机和真实 `camera/photos` Gateway/storage 均未验收。不能用历史 Android
`status/get` 构建/真机证据覆盖当前相机工作树。
